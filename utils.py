...
def update_global_nodes(to_remove: list[int]):
    global NODES
    new_nodes = []
    ...
    for node in NODES:
        ...
        for child in node.children:
            assert child in index_map, f'child {child} not in index_map'
...
def print_circuit(root_node: Node):
    ...
    for node in NODES:
        ...
        if len(node.parents) == 0 and node != root_node:
            ...
            print('remove isolated node:', node.index)
            # TODO: recursively remove the children
            def dfs(node_dfs: Node):
                if len(node_dfs.children) == 0:
                    return
                for child in node_dfs.children:
                    dfs(NODES[child])
                    NODES[child].parents.remove(node_dfs.index)
                    if len(NODES[child].parents) == 0:
                        to_remove.append(NODES[child].index)
                        print('remove child node:', NODES[child].index)
            dfs(node)
    ...
    for node in NODES:
        ...
        if len(node.parents) == 1:
            ...
            if node.type == NODES[parent].type:
                to_remove.append(node.index)
                print(node.index, node.parents, node.children)
                for child in node.children:
                    NODES[child].parents.remove(node.index)
