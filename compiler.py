...
from circuit import NodeType, create_node, Node, print_circuit
...
def free_subcircuit_compilation(ctx: context.Context, 
    ...
    for pair in unprocessed_pair:
        and_node.children.add(ctx.freepair_to_node_index[pair])
    return and_node
...
def domain_recursion(ctx: context.Context, domain: list[Const], DomainToCell: dict[Const, Cell],
    ...
    for block_idx, block in enumerate(rec_state.block_status[ego_element]):
        if not block:
            continue
        logger.info(f'{" " * rec_state.depth} block_idx: {block_idx}')
        for witness in select_witness(ego_element, block_idx, rec_state.block_status, rec_state.candidate_witness):
            logger.info(f'{" " * rec_state.depth} select witness {witness} from {rec_state.candidate_witness[ego_element][block_idx]}')
            ego_cell = DomainToCell[ego_element]
            wit_cell = DomainToCell[witness]
            
            for subcircui_index, expr, breakblock in ctx.get_wit_subcircuit((ego_cell, wit_cell), (ego_element, witness), block_idx):
